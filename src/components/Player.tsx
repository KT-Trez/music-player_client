...
export default class Player extends Component<PlayersProps, PlayerState> {
	...
	componentDidUpdate(prevProps: Readonly<PlayersProps>, prevState: Readonly<PlayerState>, snapshot?: any) {
		if (this.props.selectedAlbum && prevProps.selectedAlbum !== this.props.selectedAlbum) {
			const albumURL = process.env.REACT_APP_ORIGIN_URL ? new URL('album/content?albumID=' + this.props.selectedAlbum.id, process.env.REACT_APP_ORIGIN_URL).toString() : '/album/content?albumID=' + this.props.selectedAlbum.id;
			fetch(albumURL)
				.then(res => res.json())
				.then(songsArr => {
					this.setState({
	...
	render() {
		return (
			<Box sx={{display: 'flex', flexDirection: 'column', overflowX: 'hidden', height: 1, width: 1}}>
				<Card
					sx={{display: 'flex', alignItems: 'center', justifyContent: 'center', height: 0.35, margin: '5px'}}>

					{/* album image & data */}
					<CardContent sx={{width: 1 / 3}}>
						{this.props.selectedAlbum ?
							<CardMedia alt={'placeholder'} component='img' image={'https://placehold.jp/240x240.png'}
					...
					<CardContent
						sx={{display: 'flex', flexDirection: 'column', justifyContent: 'center', width: 2 / 3}}>
						{/* buttons controls */}
						<CardContent sx={{display: 'flex', alignItems: 'center', justifyContent: 'space-around'}}>
							{/* html audio player element */}
							<audio onEnded={this.audioEnded} onLoadedData={this.audioLoaded}
								   onTimeUpdate={this.audioUptime} ref={this.audioRef}>
								<source
									src={this.state.songSelected ? (process.env.REACT_APP_ORIGIN_URL ? 'http://localhost:5000' + this.state.songSelected.cdn : this.state.songSelected.cdn) : undefined}
									type='audio/mpeg'/>
							...
							<CardContent
								...
								<IconButton onClick={this.controlsPause}>
									<Icon
										sx={{fontSize: '60px'}}>{this.state.songIsPaused ? 'play_arrow' : 'pause'}</Icon>
								</IconButton>
								<IconButton onClick={() => this.controlsChangeSong(1)}>
							...
							<CardContent
								sx={{display: 'flex', flexGrow: 1, alignItems: 'center', justifyContent: 'center'}}>
								<IconButton onClick={this.controlsRepeat}>
									<Icon color={this.state.flags.repeat ? 'primary' : 'inherit'}
										  sx={{fontSize: '25px'}}>repeat</Icon>
								</IconButton>
								<Switch checkedIcon={<Icon sx={{fontSize: '25px'}}
														   style={{marginTop: 'calc(51.5px / 2 - 28px)'}}>play_circle_rounded</Icon>}
										onChange={this.controlsAutoplay}/>
								<IconButton onClick={this.controlsMixin}>
									<Icon color={this.state.flags.mixin ? 'primary' : 'inherit'}
										  sx={{fontSize: '25px'}}>shuffle</Icon>
								</IconButton>
							</CardContent>
						</CardContent>
						...
						<CardContent sx={{
							...
							<Slider max={this.state.time.max ? this.state.time.max : 0} min={0}
									onChange={(_, value) => this.setCurrentTime(value as number)} step={0.01}
									sx={{
										width: 0.86,
										'& .MuiSlider-thumb': {
											width: 12,
				...
				<Card sx={{height: 0.65, margin: '5px', overflow: 'auto'}}>
					<TracksList isFetching={this.state.isFetching} selectSong={this.selectSong}
								songs={this.state.songs ? this.state.songs : []}/>
				</Card>
			</Box>
		);
	}
